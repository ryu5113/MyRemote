...
class _RemotePageState extends State<RemotePage> {
  ...
  Future<void> scanPairingCode() async {
    ...
    setState(() {
      ip.text = uri!.queryParameters['host'] ?? '';
      accessKey.text = uri.queryParameters['key'] ?? '';
    });
  }
  ...
  Widget build(BuildContext context) => Scaffold(
        ...
        body: AnimatedBuilder(
          animation: remote,
          builder: (context, _) => remote.connected
              ? ConnectedScreen(remote: remote, message: message)
              : Center(
                  child: ConstrainedBox(
                    ...
                    child:
                      ...
                      const Text(
                          'PC와 휴대폰을 같은 Wi-Fi에 연결한 뒤 Agent에 표시된 IP를 입력하세요.'),
                      ...
                      FilledButton.icon(
                        ...
                        icon: Icon(
                            remote.connected ? Icons.link_off : Icons.link),
                      ...
                      OutlinedButton.icon(
                          onPressed: scanPairingCode,
                          icon: const Icon(Icons.qr_code_scanner),
                          label: const Text('QR로 자동 입력')),
                      ListTile(
                        contentPadding: EdgeInsets.zero,
                        leading: Icon(Icons.circle,
                            color:
                                remote.connected ? Colors.green : Colors.grey),
                      ...
                      if (remote.connected) ...[
                        ...
                        Wrap(spacing: 8, children: [
                          for (final delta in [-120, 120])
                            OutlinedButton(
                                onPressed: () => remote.send(
                                    {'type': 'mouse_scroll', 'delta': delta}),
                      ...
                      FilledButton(
                          onPressed: remote.connected
                              ? () => remote.send(
                                  {'type': 'keyboard', 'text': message.text})
                              : null,
