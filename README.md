...
긴 키를 입력하지 않으려면 Agent가 실행된 PC에서 `http://localhost:8765/`를 브라우저로 여세요. 연결 QR 안내 페이지가 표시됩니다. 앱의 `QR로 자동 입력`으로 PC 화면의 QR을 스캔하세요. QR 이미지 주소 `/pairing.png`는 PC 자신에서만 열립니다.

두 손가락 스크롤도 지원합니다. 연결 후에는 터치패드를 세로 스크롤 목록에서 분리한 고정 제어 화면을 표시합니다. 아직 미구현: 드래그 앤 드롭, 단축키 조합, 시스템 전원, 프로그램 실행, Wake-on-LAN, 장치 저장/자동검색, TV, Watch, Scene, 파일 공유, TLS·배포 서명. 전체 로드맵의 완성 버전으로 표시하지 않습니다.
